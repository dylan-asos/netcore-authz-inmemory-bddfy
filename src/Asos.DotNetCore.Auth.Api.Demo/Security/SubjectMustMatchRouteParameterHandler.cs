...
public class SubjectMustMatchRouteParameterHandler : AuthorizationHandler<SubjectMustMatchRouteParameterRequirement>
{
    ...
    private bool DoRouteValueAndClaimTypeValueMatch(ClaimsPrincipal principal, ActionContext actionContext,
        string routeParameterName, string claimsType)
    {
    ...
    private bool HasClaimAndValue(ClaimsPrincipal principal, string claimsType, string expectedClaimValue)
    {
        var claimValue = principal.Claims.FirstOrDefault(x => x.Type == claimsType);

        return claimValue != null &&
               claimValue.Value.Equals(expectedClaimValue, StringComparison.CurrentCultureIgnoreCase);
    }
}